...
for (int BC_type = 1; BC_type < 4; BC_type++)
{
    ...
    {
        /* Staggering the initial B_z field by one time-step */
        if(qTime == 0 && BC_type < 3)
        {
            for(mm = 0; mm < SIZE; mm++)
            {
                if(abs(x[2 * mm]) < 0.1) B_z[mm] = sin(20. * M_PI * x[2 * mm]);
                else B_z[mm] = 0.;
            }
        ...
        case 1:
            ...
            E_y[0]        = E_y[0]        - (tau/(2*h))*(B_z[0] - B_z[SIZE - 2]);
            E_y[SIZE - 1] = E_y[SIZE - 1] - (tau/(2*h))*(B_z[0] - B_z[SIZE - 2]);
            break;
        case 2:
            ...
            break;
        case 3:
            /* Perfect boundary conditions. Maybe also check Mur's boundary conditions! */
            if(qTime > 0)
            {
                E_y[0]        = E_y[0]        - (tau/(2*h))*(B_z[0] - Res_B[(qTime + 1) - 1][0]);
                E_y[SIZE - 1] = E_y[SIZE - 1] - (tau/(2*h))*(Res_B[(qTime + 1) - 1][SIZE - 2] - B_z[SIZE - 2]);
            }
            break;
        }
        ...
        /* Results */
        if (qTime % 1 == 0) // 'stride' was changed to 4 for now.
        {
